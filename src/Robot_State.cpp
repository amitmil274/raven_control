...
Robot_State::Robot_State()
{
	...
	pthread_mutexattr_init(&data2MutexAttr);
	pthread_mutexattr_setprotocol(&data2MutexAttr,PTHREAD_PRIO_INHERIT);
	pthread_mutex_init(&data2Mutex,&data2MutexAttr);
 ...
 *	\return bool
 */
bool Robot_State::set_Radius(int radius)
{
 ...
 *	\return bool
 */
bool Robot_State::set_Speed(int speed)
{
...
bool Robot_State::set_Scale(int scale)
{
 ...
 *	\return bool
 */
bool Robot_State::set_Direction(int direc)
{
	if(direc != 1 && direc != -1)
	{	
 ...
 *	\return bool
 */
bool Robot_State::set_BasePlane(int plane)
{
 ...
 *	\return bool
 */
bool Robot_State::set_ArmType(int armtype)
{
...
bool Robot_State::set_DeviceType(int devicetype)
{
	bool deviceOK = devicetype == RAVEN || devicetype == HAPTIC;
 ...
 *	\return bool
 */
bool Robot_State::set_Center(boost::array<int, 6> center,boost::array<float, 2> center_grasp)
{
...
bool Robot_State::set_Center(boost::array<float, 6> center,boost::array<float, 2> center_grasp)
{
	tfScalar X,Y,Z,Grasp;
 ...
 *	\return bool
 */
bool Robot_State::set_Current_Pos(boost::array<int, 6> currpos)
{
	tfScalar X,Y,Z;
 ...
 *	\return bool
 */
bool Robot_State::set_Current_Ori(boost::array<float, 18> rot_matix)
{
 ...
 *	\return tfScalar
 */
tfScalar Robot_State::sign(tfScalar value)
{
	if(value >= 0)
		return 1;
	else 
...
//tfScalar Robot_State::get_Current_grasp(){
//	return ((Current_Grasp-Previous_Grasp)*1000);
//}
 ...
 *	\return void
 */
void Robot_State::checkPathState()
{
 ...
 *	\return tfScalar
 */
tfScalar Robot_State::get_Radius()
{
 ...
 *	\return tfScalar
 */
tfScalar Robot_State::get_Radius_Range()
{
	// [DANGER]: related to modification parameter tuning
 ...
 *	\return tfScalar
 */
tfScalar Robot_State::get_Speed()
{
	tfScalar SP = Speed*ROS_PUBLISH_RATE/10000; // in cm/sec
	return SP;
}
tfScalar Robot_State::get_Scale()
{
	return Scale;
}
 ...
 *	\return void
 */
void Robot_State::show_PathState()
{
	if(ArmType == LEFT_ARM)
	{
 ...
 *	\return void
 */
void Robot_State::show_Distance()
{
	if(ArmType == LEFT_ARM)
	{
...
void Robot_State::show_Center()
{
	...
	if(DeviceType == RAVEN)
		if(ArmType == LEFT_ARM)
		{
			cout<<"\tRaven Center[LEFT] = ("<<x<<","<<y<<","<<z<<")"<<endl;

		}
		else if(ArmType == RIGHT_ARM)
		{
			cout<<"\tRaven Center[RIGHT] = ("<<x<<","<<y<<","<<z<<")"<<endl;
		}
	if(DeviceType == HAPTIC)
		if(ArmType == LEFT_ARM)
		{
			cout<<"\tHaptic Center[LEFT] = ("<<x<<","<<y<<","<<z<<")"<<endl;

		}
		else if(ArmType == RIGHT_ARM)
		{
			cout<<"\tHaptic Center[RIGHT] = ("<<x<<","<<y<<","<<z<<")"<<endl;
		}
}
 ...
 *	\return tfScalar
 */
tfScalar Robot_State::DistanceOf(tf::Vector3 point1,tf::Vector3 point2)
{
 ...
 *	\return tf::Transform
 */
tf::Transform Robot_State::ComputeCircleTrajectory()
{
...
force_feedback Robot_State::ComputeForces(Robot_State ravenArm, haptic_locks lock_state)
{
	...
	forces.force_grip = 0;
	forces.force_trans.setValue(0.0,0.0,0.0);
	...
	{
		forces.force_trans = - KP * (this->Current_Pos-this->Center);
	...
	{
		forces.force_grip = - KG * (this->Current_Grasp-this->CenterGrasp);
		forces.force_grip   -= KVG * Velocity_Grasp;
	}
 ...
 *	\return tf::Transform
 */
tf::Transform Robot_State::ComputeNullTrajectory()
{
	tf::Transform TF_INCR;
 ...
 *	\return void
 */
void Robot_State::AutoCircleMotion1()
{
 ...
 *	\return void
 */
void Robot_State::AutoCircleMotion2()
{
	tfScalar    del_angle;
 ...
 *	\return void
 */
void Robot_State::AutoCircleMotion3()
{
 ...
 *	\return tf::Vector3
 */
tf::Vector3 Robot_State::AutoCircleMotion4()
{
	tfScalar del_angle;
 ...
 *	\return tf::Vector3
 */
tf::Vector3 Robot_State::TuneRadiusMotion()
{
...
bool Robot_State::set_Modi_Scale(int x)
{
...
bool Robot_State::set_Modi_Speed_Pow(int x)
{
	tfScalar step = 0.1;
...
bool Robot_State::set_Modi_Dista_Pow(int x)
{
	tfScalar step = 0.5;
...
tfScalar Robot_State::get_Modi_Scale()
{
	tfScalar result = Modi_Scale;
	return result;
}

tfScalar Robot_State::get_Modi_Speed_Pow()
{
	tfScalar result = Modi_Speed_Pow;
	return result;
}

tfScalar Robot_State::get_Modi_Dista_Pow()
{
